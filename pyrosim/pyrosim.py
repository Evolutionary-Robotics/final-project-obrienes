...
def Send_Cube(name="default",pos=[0,0,0],size=[1,1,1],shape="cube"):
    ...
    if filetype == SDF_FILETYPE:
        ...
        link = LINK_URDF(name,pos,size,shape)
...
def Send_Cylinder(name="default",pos=[0,0,0],orientation=[0,0,0],size=[1,1,1],shape="cylinder"):
    ...
    if filetype == SDF_FILETYPE:
        ...
        link = LINK_URDF(name,pos,orientation,size,shape)
...
def Set_Motor_For_Joint(bodyIndex,jointName,controlMode,targetPosition=0,maxForce=0,maxVelocity=0,targetVelocity=0):
