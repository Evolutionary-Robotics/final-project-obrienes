...
class Microbial():
    ...
    def showFitness(self):
        ...
        plt.plot(self.avgHistory[:self.gen+1])
        np.save("bestHistory.npy",self.bestHistory[:self.gen+1])
        np.save("avgHistory.npy",self.avgHistory[:self.gen+1])
        plt.xlabel("Generations")
        plt.ylabel("Fitness")
        plt.title("Best and average fitness")
        plt.show()
    ...
    def fitStats(self):
        if np.max(self.fitness) > self.bestFitness:
            self.bestFitness = np.max(self.fitness)
    ...
    def run(self):
        ...
        for g in range(self.generations):
            ...
            for i in range(self.popsize):
                ...
                # Step 2: Update fitness
                # forward = np.random.randint(-180, 180)
                # Step 3: Compare their fitness
